...
private struct ResearchExperimentView: View {
                ...
                ScrollView {
                    VStack(alignment: .leading, spacing: 14) {
                        ...
                        if operation == "sae" {
                            ...
                            Text("TopK retains at most k positive features per example. Set top_k in parameters; default 8. This small sandbox is not a pretrained SAELens model.").font(.caption).foregroundStyle(.secondary)
                        }
                        resourceStatus
    ...
    private var description: String {
        switch operation {
        case "patch_sweep": return "Patch one layer/token site at a time from a clean prompt into an equally tokenized corrupted prompt. Compare target-minus-foil logits; edit layers and positions below (maximum 128 sites)."
    ...
    @ViewBuilder private func resultView(_ result: [String: Any]) -> some View {
        ...
        if let patches = result["patches"] as? [[String: Any]] {
            ...
            ForEach(patches.indices, id: \.self) { i in
                let row = patches[i]
                Text("Layer \(row["layer"] as? Int ?? 0), token \(row["position"] as? Int ?? 0): Δ \(row["delta"] as? Double ?? 0), recovery \((row["recovery"] as? Double).map { String(format: "%.3f", $0) } ?? "undefined")").font(.caption.monospaced())
            }
        }
    ...
    private static func template(_ operation: String) -> String {
        ...
        else if operation == "patch_sweep" {
            config.merge(["layers": [0, 1], "prompt": "The capital of Italy is", "clean_prompt": "The capital of France is", "target_token": " Paris", "foil_token": " Rome", "positions": [3, 4]]) { _, new in new }
        }
